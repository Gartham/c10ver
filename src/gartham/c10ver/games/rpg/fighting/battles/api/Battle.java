 ...
 * of at least one {@link SimpleFighter}. {@link Battle}s are <b>turn-based</b>, with
 * each turn being taken through the doing of an <b>action</b> by a
 * {@link SimpleFighter}. Each action "takes" a certain number of "ticks", which are,
 ...
 * {@link SimpleFighter}'s turn to make its next move. Upon the conclusion of any
 * turn, if all {@link SimpleFighter}s on a {@link Team} are considered dead (i.e.
 ...
 * {@link SimpleFighter} had the last turn.
 * </p>
 * <p>
 * Each turn, a {@link SimpleFighter} takes an <b>action</b>. Actions take a certain
 ...
 * adding "effects" that target {@link SimpleFighter}s, which are kept track of by the
 * {@link Battle} and cease their effects after a certain number of ticks,
 * affecting the targeted {@link SimpleFighter} while extant). {@link Battle}
 * subclasses should expose the actions that can be taken by a {@link SimpleFighter}
 ...
 *            {@link SimpleFighter}s tracked by this {@link Battle}.
 */
public abstract class Battle<A> {

	private final Map<SimpleFighter, Integer> ticksTillTurn = new HashMap<>();
	private final List<SimpleFighter> battleQueue = new ArrayList<>();
	...
	/**
	 * <p>
	 * This is the method that assigns <b>initial ticks</b> to each {@link SimpleFighter}.
	 * Upon a call to this method by {@link #start()}, the provided
	 * <code>queue</code> of {@link SimpleFighter}s (1) contains all of the
	 * {@link SimpleFighter}s participating in the {@link Battle} and (2) is sorted, in
	 * descending order, by {@link SimpleFighter#getSpeed() speed}. This means that the
	 * {@link SimpleFighter} with the highest speed is positioned at index <code>0</code>.
	 * </p>
	 * <p>
	 * This method is <b>only</b> tasked with setting the number of ticks for each
	 * {@link SimpleFighter} through the {@link #setTicks(SimpleFighter, int)} method. The
	...
	/**
	 * Acts as the current {@link SimpleFighter}.
	 * 
	 ...
	 * <b>not</b> add said number of ticks to the {@link SimpleFighter} that performed the
	 * action. That is handled by {@link #act(Object)} in the {@link Battle} class.
	 * 
	 * @param action  The action taken.
	 * @param fighter The {@link SimpleFighter} that took the action (i.e. the current
	 *                fighter). At the beginning of this method call, (as per normal
	 *                {@link Battle} behavior), this argument should be exactly the
	 *                same as the {@link SimpleFighter} in the front (position
	 ...
	 * conclusion of every turn, specifically after the ticks to the {@link SimpleFighter}
