 ...
 * {@link GarmonTeam#getController()} method to perform the operations normally
 * done by calling code in between calls to {@link #act(int)}). As a result,
 * this class provides multiple modes of operation, including modes executing on
 * their own, discrete threads. (Please note that this class is not multi-thread
 * safe; in the case of methods which launch their own thread, they are expected
 * to be the only thread to interface with the {@link GarmonBattle} until its
 * completion.)
 * 
 * @author Gartham
 *
